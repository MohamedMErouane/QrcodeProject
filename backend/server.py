    ...
    if parsed_url.path == '/api/absent-dates':
        try:
            ...
            with conn.cursor() as cur:
                absent_dates_query = """
                SELECT DISTINCT TO_CHAR(s."sessionDate", 'YYYY-MM-DD') AS "absentDate"
                FROM "Session" s
                WHERE s.id NOT IN (
                    SELECT "sessionId"
                    FROM "Attendance"
                ...
                """
                cur.execute(absent_dates_query)
                absent_dates = [row[0] for row in cur.fetchall()]

            self._send_response(200, {"absentDates": absent_dates})
            conn.close()
        ...
        try:
            ...
            if not date:
                self._send_response(400, {"error": "Date parameter is required"})
                return
            ...
            with conn.cursor() as cur:
                ...
                SELECT TO_CHAR(s."sessionDate", 'YYYY-MM-DD') AS "sessionDate",
                       s."topic" AS "session",
                       s."professor" AS "professor"
                FROM "Session" s
                WHERE TO_CHAR(s."sessionDate", 'YYYY-MM-DD') = %s;
                """
                cur.execute(session_details_query, (date,))
                session_details = cur.fetchone()

                if session_details:
                    response_data = {
                        "session": session_details[1],
                        "professor": session_details[2]
                    }
